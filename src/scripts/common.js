...
const scriptPath = document.currentScript.src; // Get the path of the current script
const basePath = scriptPath.substring(0, scriptPath.lastIndexOf('/')); // Get the base path
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (navbar) {
    fetch('/src/nav.html')  // adjust path if file is in a subfolder (e.g., '../nav.html')
      .then(res => res.text())
